...
class Left_menu
{
    ...
    private  function getAllSubModuleByModuleId($id = null)
    {
        ...
        foreach ($subModules as $key => $sval) {
            $sub_sub_menu = [];
            $sub_sub_module_obj = new SubSubmodules();
            $sub_sub_modules = $sub_sub_module_obj->findAll();

            if (count($sub_sub_modules) > 0) {
                foreach ($sub_sub_modules as $subkey => $ssm_val) {

                    if ($ssm_val['sm_id'] == $sval['sm_id']) {
                        $sub_menu_items = ["name" => $ssm_val['ssm_name'], "url" => $ssm_val['ssm_url'], "class" => $ssm_val['ssm_class']];
                        $sub_sub_menu[] = $sub_menu_items;
                    }
                }
            }
