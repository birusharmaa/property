...
class Leads extends Security_Controller
{
    ...
    public function add_leads(){
        ...
        $data['user_lists'] = $this->Users->findAll();

        $this->states->select(['id', 'name']);
        $data['states'] = $this->states->where('country_id', '101')->findAll();
    ...
    function addLeads(){
        ...
        $this->validation->setRules([
            'category' => ['label' => 'category', 'rules' => 'trim|required'],
            // 'lastName[]'  => ['label' => 'firstName', 'rules' => 'required'],
            //'email[]'     => ['label' => 'email', 'rules' => 'valid_email'],
            // 'number[]'    => ['label' => 'firstName', 'rules' => 'required'],
        ]);
        ...
        if($this->validation->withRequest($this->request)->run()){  
                    ...
                    $sub_category    = $this->request->getPost('subcategory1');
                    ...
                    $commercial      = $this->request->getPost('commercial');
    ...
    function show_lead($id=null){
        $this->leads->select('users.name AS posted_by_name, cities.name AS city_name, states.name AS state_name');
        $this->leads->select('leads.*');
        $this->leads->join('cities', 'leads.city=cities.id', 'LEFT');
        $this->leads->join('states', 'leads.state=states.id', 'LEFT');
        $this->leads->join('users', 'leads.posted_by=users.id', 'LEFT');
        $data = $this->leads->find($id);   
        $result['lead_details'] = $data;
        if($data){
    ...
    function getsubcategory(){
        //$catId = !empty($this->request->getVar('catId')?$this->request->getVar('catId'):0);
