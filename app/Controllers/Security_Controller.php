...
class Security_Controller extends AppController {
    ...
    public function __construct($redirect = true) {
        ...
        $this->login_user = $this->User->get_access_info($login_user_id);
    ...
    protected function validate_sending_message($to_user_id) {
        ...
        $to_user_info = $this->User->get_one($to_user_id);
        if ($to_user_info->user_type == "staff") {
            //receiver is a team member
            $permissions = array();
            $user_permissions = $this->User->get_access_info($to_user_id)->permissions;
            if ($user_permissions) {
    ...
    protected function get_team_members_dropdown($is_filter = false) {
        $team_members = $this->User->get_all_where(array("user_type" => "staff", "deleted" => 0, "status" => "active"))->getResult();
